...
<html lang="en">
    <head>
        ...
        <title>Volunteer | Eagle Flight Chaplain</title>
    ...
    <body>
        ...
        <section class="frontPage" id="orderPage">
        ...
        <div class="banner">
            <img src="images/eagle_flight_chaplain_banner2.jpg" alt="Join eagleflight chaplain">
        </div>
        <div class="tag">
            <h2>Please fill the form below</h2>
            <hr>
            <form method="POST" action="join_us.php" id="orderForm">
                <select name="event" id="event">
                    <option value=""selected>Select Event</option>
                    <?php
                        $get_events = $connectdb->prepare("SELECT title FROM events WHERE event_date > CURDATE() ORDER BY event_date DESC");
                        $get_events->execute();
                        $events = $get_events->fetchAll();
                        foreach($events as $event):
                    ?>
                    <option value="<?php echo $event->title?>"><?php echo ucwords($event->title)?></option>  
                    <?php endforeach?>
                </select>
                ...
                <button type="submit" name="volunteer" id="order">Join</button>
            </form>
        </div>
